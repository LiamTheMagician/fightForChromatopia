        ...
        for i in range(len(self.matrix)):
            for j in range(len(self.matrix[i])):
                ...
                if self.matrix[i][j] == 'N':
                    n = Tile((j*GRID_SIZE, i*GRID_SIZE))
                    self.npc.append(n)
                if self.matrix[i][j] == 'P':
                    self.p = Player(player_speed, (j*GRID_SIZE,i*GRID_SIZE),self.obstacles+self.npc, self.npc, self.screen)
                    self.players.append(self.p)
        self.group.add(self.players, self.obstacles, self.npc)
